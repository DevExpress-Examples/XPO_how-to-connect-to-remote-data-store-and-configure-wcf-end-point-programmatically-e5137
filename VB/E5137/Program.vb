        ...
        Shared Sub Main()
            ...
            Using uow As UnitOfWork = New UnitOfWork()
                If uow.FindObject(GetType(Customer), New BinaryOperator("ContactName", "Alex Smith", BinaryOperatorType.Equal)) Is Nothing Then
                    Dim custAlex As Customer = New Customer(uow) With {.ContactName = "Alex Smith", .CompanyName = "DevExpress"}
                    custAlex.Save()
                    Dim Tom As Customer = New Customer(uow) With {.ContactName = "Tom Jensen", .CompanyName = "ExpressIT"}
                    Tom.Save()
                    uow.CommitChanges()
                End If
